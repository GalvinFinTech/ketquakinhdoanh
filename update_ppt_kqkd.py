...
def fmt_vn(val, is_percent=False):
    ...
    - Ví dụ: 1234.56 -> 1.234,56 | 98.86% -> +98,86%
    """
    ...
    # Safety fallback: Nếu giá trị phần trăm vẫn ở dạng decimal fraction (0.9886 -> 98.86%)
    if is_percent and abs(f_val) <= 2.0 and f_val != 0:
        f_val = f_val * 100.0
...
def update_powerpoint_kqkd():
    ...
    1. Lấy dữ liệu Top 30 Doanh nghiệp Vốn hóa Lớn & Vừa Tăng Trưởng Dương (>0%).
    ...
    try:
        ...
        print("❌ Không thể mở file PowerPoint. Hãy đảm bảo bạn đã ĐÓNG hoàn toàn PowerPoint!")
    ...
    try:
        ...
        print("❌ KHÔNG THỂ LƯU FILE: Hãy đảm bảo bạn đã ĐÓNG file 'Morning Note_26.pptx' trước khi chạy script!")
        print(f"Chi tiết lỗi: {e}")
        return False
